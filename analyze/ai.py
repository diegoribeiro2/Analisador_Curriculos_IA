import re  # usada para operações com expressões regulares
...
class OpenAIClient:
    def __init__(self, model_id="gpt-5-mini"):
        # Inicializar o modelo de linguagem com o ID especificado
        self.model_id = model_id
        self.client = ChatOpenAI(model=self.model_id,
                                 temperature=1,
                                 openai_api_key=os.getenv("OPENAI_API_KEY"))  # pega da variável de ambiente
    ...
    def resume_cv(self, cv):
        ...
        try:
            result = result_raw.split('```markdown')[1]   # Pega o conteúdo após '```markdown'
        except:
            # Se não conseguir, retornar a resposta bruta
            result = result_raw
        return result

    def generate_score(self, cv, job, max_attempts=10):  # gera score, usando 10 tentativas
